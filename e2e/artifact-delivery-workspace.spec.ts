...
import type { ArtifactPreview } from '../src/features/deliverables/artifactApi';
...
const result = {
  ...
  version_number: 1, version_id: '40000000-0000-0000-0000-000000000001',
  artifact_label: '客户解决方案', download_urls: {},
  requested_formats: ['docx', 'pdf', 'xlsx'], verification_items: [],
  ...
  test(`result preview and revision: ${scenario.name}`, async ({ page }, testInfo) => {
    ...
    await page.route('**/api/artifacts/artifact-preview-test/revisions', async (route) => {
      expect(route.request().postDataJSON()).toMatchObject({
        instructions: '补充安装培训和售后边界',
        base_version_id: result.version_id,
        section_heading: '售后边界',
      });
    ...
    await dialog.getByRole('combobox', { name: '修订范围' }).selectOption('售后边界');
    ...
    await expect(dialog.getByRole('combobox', { name: '修订范围' })).toHaveValue('售后边界');
    await dialog.getByText('修改要求', { exact: true }).click();
    ...
    await dialog.getByText('修改要求', { exact: true }).click();
    await dialog.getByLabel('修订要求').fill('补充安装培训和售后边界');
    await expect(dialog.getByRole('button', { name: '生成修订稿' })).toBeEnabled();
