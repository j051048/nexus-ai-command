...
2. 建立每日数据库备份，并在隔离 staging 完成一次恢复演练；生产环境必须设置 `BACKUP_STORAGE_BACKEND`（`filesystem` 或 `s3`），否则应用级备份与主库同生共死。
3. 在 staging 用真实资料跑一次 `scripts/record_artifact_outputs.py --from-pipeline`，把产物质量基线升级为 `live-model`；在此之前不得对外承诺"一次通过率 ≥90%、平均分 ≥85"。
4. 与客户确认资料分类、可信来源、企业模板、品牌规范、外发审批人和禁止自动承诺的条款。
5. 每日检查企业资料入库失败、低证据覆盖成果、质量门失败、工具失败与 Token 异常。
6. 只对真实要用的集成配置生产凭据；未配置的集成应 fail closed 并显示可操作提示。
7. 记录首份方案/标书的生成耗时、人工修改量、采用结果和客户反馈，作为后续质量基线。
