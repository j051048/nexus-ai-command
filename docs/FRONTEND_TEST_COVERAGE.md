...
## 1. 现状（2026-09-21 开发机快照）

从 `coverage/coverage-summary.json` 读取:

| 指标 | 当前 | 文件数 | 备注 |
|---|---:|---:|---|
| 行覆盖 | 15.50% | 当前完整 `src` 统计 | |
| 语句覆盖 | 14.94% | 当前完整 `src` 统计 | |
| 函数覆盖 | 11.53% | 当前完整 `src` 统计 | |
| 分支覆盖 | 11.28% | 当前完整 `src` 统计 | |
| 测试规模 | 436 个测试 / 71 个测试文件 | | 全部通过 |

本次快照来自 `npm run test -- --coverage`。百分比上升有两个来源：新增的关键路径契约测试，以及删除了 47KB 未被引用的 `src/lib/i18n.ts` 及其 4 个 locale 文件（分母下降 4,700+ 行）。**这不是测试资产的同比例增长**，整体覆盖仍低。

测试主要集中在关键 Hook、纯规则、错误边界、布局契约、方案/投标状态模型和少量组件。大型管理页、企业资料页及成果生成 UI 仍缺足够的组件级分支覆盖；`ArtifactQualityOperationsPage` 等运营看板仍只有间接覆盖。
