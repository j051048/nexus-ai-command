"""Static P0-P6 release quality gate.

This script intentionally avoids network and database access. It verifies that
the repository still contains the launch-critical controls that are easy to
regress during fast product work.
"""

from __future__ import annotations

import sys
from dataclasses import dataclass
from pathlib import Path

ROOT = Path(__file__).resolve().parents[1]


@dataclass(frozen=True)
class GateCheck:
    level: str
    name: str
    path: str
    tokens: tuple[str, ...] = ()
    severity: str = "critical"


def read_text(path: str) -> str:
    return (ROOT / path).read_text(encoding="utf-8", errors="replace")


def file_exists(path: str) -> bool:
    return (ROOT / path).exists()


def run_check(check: GateCheck) -> tuple[bool, str]:
    target = ROOT / check.path
    if not target.exists():
        return False, f"missing file: {check.path}"
    if not check.tokens:
        return True, ""
    content = read_text(check.path)
    missing = [token for token in check.tokens if token not in content]
    if missing:
        return False, f"missing token(s) in {check.path}: {', '.join(missing)}"
    return True, ""


CHECKS = [
    # P0: production survivability and launch blast-radius controls.
    GateCheck(
        "P0",
        "production readiness script",
        "scripts/production_readiness_check.mjs",
        ("developer tools disabled", "golden smoke route:"),
    ),
    GateCheck(
        "P0",
        "production health checker",
        "scripts/production_health_check.mjs",
        ("/health/live", "/health/ready", "HEALTH_CHECK_TOKEN"),
    ),
    GateCheck(
        "P0",
        "static RLS coverage scanner",
        "scripts/scan_rls_coverage.py",
        ("MISSING_POLICY", "_collect_policy_tables"),
    ),
    GateCheck(
        "P0",
        "migration schema conflict scanner",
        "scripts/scan_migration_schema_conflicts.py",
        ("CREATE TABLE IF NOT EXISTS", "schema-compatible", "extract_definitions"),
    ),
    GateCheck(
        "P0",
        "RLS policy column scanner",
        "scripts/scan_rls_policy_columns.py",
        ("current_tenant_id_text", "TENANT_COLUMN_RE", "is not defined"),
    ),
    GateCheck(
        "P0",
        "scratch database migration replay",
        "scripts/verify_migration_replay.py",
        ("MIGRATION_REPLAY_DATABASE_URL", "ON_ERROR_STOP=1", "psql"),
    ),
    GateCheck(
        "P0",
        "local Python runtime launcher",
        "scripts/dev_python.ps1",
        (".venv", "trying global Python", "No Python runtime found"),
    ),
    GateCheck(
        "P0",
        "containerized backend build",
        "Dockerfile",
        ("COPY nexus_backend/requirements.txt", "USER appuser"),
    ),
    GateCheck(
        "P0",
        "local deployment composition",
        "docker-compose.yml",
        ("celery-worker", "redis"),
    ),
    GateCheck(
        "P0",
        "customer launch module manifest",
        "src/config/customerLaunchModules.ts",
        ("owner:", "smokePath:"),
    ),
    GateCheck(
        "P0",
        "top 10 E2E smoke suite",
        "e2e/top10-critical-flows.spec.ts",
        ("/login", "/crm", "/approval", "/finance", "/workflows"),
    ),
    GateCheck(
        "P0",
        "document embedding vector index",
        "supabase/migrations/20260514_p0_document_embeddings_vector_index.sql",
        ("vector_cosine_ops",),
    ),
    GateCheck(
        "P0",
        "tenant RLS policy backfill",
        "supabase/migrations/20260514_p0_tenant_rls_policy_backfill.sql",
        ("current_tenant_id_text", "CREATE POLICY", "organization_id::text"),
    ),
    GateCheck(
        "P1",
        "known failure regression contracts",
        "nexus_backend/tests/production_proof/test_known_failure_regressions.py",
        ("_kingdee_identity", "PROMPT_FIREWALL_LLM_JUDGE", "chat-input"),
    ),
    GateCheck(
        "P2",
        "product focus and observability contract",
        "nexus_backend/tests/production_proof/test_product_focus_and_observability_contract.py",
        ("MODULE_TIER_LABELS", "reward_model", "audit_summary"),
    ),
    GateCheck(
        "P0",
        "immutable audit log trigger",
        "supabase/migrations/20260419_p1_audit_logs_immutable.sql",
        ("prevent_audit_log_mutation", "BEFORE UPDATE OR DELETE"),
    ),
    GateCheck(
        "P0",
        "celery backlog health",
        "nexus_backend/app/core/celery_queue_monitor.py",
        ("CELERY_QUEUE_DEPTH_WARNING", "observe_celery_queue_depth"),
    ),
    # P1: operational correctness, cost visibility, and UX hardening.
    GateCheck(
        "P1",
        "bounded idempotency fallback",
        "nexus_backend/app/core/idempotency_middleware.py",
        ("IDEMPOTENCY_MEMORY_FALLBACK_MAX", "process-local memory fallback"),
    ),
    GateCheck(
        "P1",
        "streaming usage estimation fallback",
        "nexus_backend/app/services/llm_adapters/openai_compatible.py",
        ("_estimate_stream_usage", '"estimated": True'),
    ),
    GateCheck(
        "P1",
        "route suspense and error boundary",
        "src/components/common/ModuleRouteBoundary.tsx",
        ("Suspense", "ModuleErrorBoundary", "ModuleRouteSkeleton"),
    ),
    GateCheck(
        "P1",
        "agent observability API",
        "nexus_backend/app/routers/agent_observability.py",
        ("/quality/trends", "/shadow-eval", "/context-ablation"),
    ),
    GateCheck(
        "P1",
        "semantic Tool RAG binding",
        "nexus_backend/app/agent/plan/tool_binding.py",
        ("tool_embedding_index.retrieve",),
    ),
    GateCheck(
        "P1",
        "tool governance API",
        "nexus_backend/app/routers/chat.py",
        ("/tools/governance", "/tools/rag/evaluate"),
    ),
    GateCheck(
        "P1",
        "tool developer guide",
        "docs/TOOL_DEVELOPMENT_GUIDE.md",
        ("required_role", "risk", "Tool RAG"),
    ),
    # P2: enterprise sales readiness without requiring external vendors.
    GateCheck(
        "P2",
        "enterprise SSO routes",
        "nexus_backend/app/routers/enterprise_sso.py",
        ("oidc", "saml"),
    ),
    GateCheck(
        "P2",
        "enterprise SSO signed state",
        "nexus_backend/app/services/enterprise_sso_service.py",
        ("sign_state", "verify_state", "parse_saml_response"),
    ),
    GateCheck(
        "P2",
        "API key hard-fail middleware",
        "nexus_backend/app/core/api_key_middleware.py",
        ("Invalid API key", "never silently downgraded"),
    ),
    GateCheck(
        "P2",
        "deny-by-default tool RBAC",
        "nexus_backend/app/core/tool_rbac.py",
        ("Deny-by-default guard", "_SAFE_ALL_PREFIXES"),
    ),
    GateCheck(
        "P2",
        "compliance audit export API",
        "nexus_backend/app/routers/compliance.py",
        ("/audit/export", "csv"),
    ),
    GateCheck(
        "P2", "SOC2 controls map", "docs/SOC2_CONTROLS.md", ("CC6", "CC7", "Evidence")
    ),
    GateCheck(
        "P2",
        "managed cost report RPC",
        "supabase/migrations/20260514_p2_cost_report_rpc.sql",
        ("get_cost_report", "SECURITY DEFINER"),
    ),
    GateCheck(
        "P2",
        "prompt fuzz security tests",
        "nexus_backend/tests/security/test_fuzz_api.py",
        ("@given", "prompt"),
    ),
    # P3-P6: sustained production quality controls.
    GateCheck(
        "P3",
        "Supabase client import is chunk-stable",
        "src/lib/httpClient.ts",
        ("import { supabase }", "supabase.auth.getSession"),
    ),
    GateCheck(
        "P3",
        "bundle budget script",
        "scripts/check_bundle_budget.mjs",
        (
            "maxJsChunkBytes",
            "maxTotalJsBytes",
            "maxJsChunkCount",
            "vendor-jspdf-",
            "vendor-html2canvas-",
        ),
    ),
    GateCheck(
        "P4",
        "bundle budget package command",
        "package.json",
        ('"check:bundle"', "check_bundle_budget.mjs"),
    ),
    GateCheck(
        "P4",
        "bundle budget wired to CI",
        ".github/workflows/ci.yml",
        ("Check bundle budget", "npm run check:bundle"),
    ),
    GateCheck(
        "P5",
        "private deployment doctor",
        "scripts/private_deploy_doctor.py",
        ("--env", "PRIVATE_DEPLOYMENT", "CORS_ORIGINS"),
    ),
    GateCheck(
        "P6",
        "release evidence collector",
        "scripts/collect_release_evidence.py",
        ("release-evidence.json", "sha256", "<redacted>"),
    ),
    GateCheck(
        "P1",
        "private PgBouncer guidance",
        "docs/PRIVATE_DEPLOYMENT_PGBOUNCER.md",
        ("pool_mode", "SUPABASE_DB_POOLER_URL", "/health/ready"),
    ),
    GateCheck(
        "P1",
        "small-company load profile",
        "nexus_backend/tests/k6/small_company.js",
        ("small_company_20_50_users", "p(95)<900"),
    ),
    GateCheck(
        "P1",
        "nightly agent replay runner",
        "scripts/agent_replay_nightly.py",
        ("AGENT_REPLAY_BASE_URL", "promote-failures"),
    ),
    GateCheck(
        "P1",
        "nightly agent quality workflow",
        ".github/workflows/nightly-agent-quality.yml",
        ("schedule:", "agent_replay_nightly.py"),
    ),
    GateCheck(
        "P1",
        "SOC2 evidence collector",
        "scripts/collect_soc2_evidence.py",
        ("CC6_logical_access", "CC7_security_monitoring"),
    ),
    GateCheck(
        "P1",
        "rate limiter production fail-closed",
        "nexus_backend/app/core/rate_limiter.py",
        ("_redis_is_required", "backend_unavailable"),
    ),
    GateCheck(
        "P1",
        "API public route auth matrix",
        "nexus_backend/app/core/api_auth_matrix.py",
        ("PUBLIC_API_ROUTE_REASONS", "/api/metrics/web-vitals", "/api/oauth/token"),
    ),
    GateCheck(
        "P1",
        "API auth matrix regression test",
        "nexus_backend/tests/unit/test_api_auth_matrix.py",
        (
            "test_every_dependency_free_api_route_is_explicitly_allowlisted",
            "test_public_api_route_allowlist_has_no_stale_entries",
        ),
    ),
    GateCheck(
        "P1",
        "tenant scoped DB dependency",
        "nexus_backend/app/core/dependencies.py",
        ("def get_request_db", "request.state", "status_code=401"),
    ),
    GateCheck(
        "P1",
        "ASGI unhandled exception boundary",
        "nexus_backend/app/core/security_middleware.py",
        ("class UnhandledExceptionMiddleware", "Unhandled request exception"),
    ),
    GateCheck(
        "P1",
        "unhandled exception boundary wiring",
        "nexus_backend/app/startup/middleware.py",
        ("UnhandledExceptionMiddleware", "add_middleware"),
    ),
    GateCheck(
        "P1",
        "tenant org auth regression test",
        "nexus_backend/tests/unit/test_auth_org_context.py",
        (
            "test_missing_org_without_auth_is_401",
            "test_missing_org_after_auth_failure_is_401",
        ),
    ),
    GateCheck(
        "P1",
        "prompt firewall fast-path regression test",
        "nexus_backend/tests/unit/test_prompt_firewall_fast_path.py",
        ("test_context_overflow_does_not_call_llm_judge", "context_overflow"),
    ),
    GateCheck(
        "P1",
        "customer acceptance mock auth contract",
        "e2e/customer-business-acceptance.spec.ts",
        (
            "mockLoggedInState(page, role)",
            "getByTestId('chat-input')",
            "toHaveURL(/\\/dashboard/",
        ),
    ),
    GateCheck(
        "P1",
        "unified action inbox API",
        "nexus_backend/app/routers/inbox.py",
        (
            "class ActionItem",
            "_approval_to_action_item",
            "_customer_risk_to_action_item",
        ),
    ),
    GateCheck(
        "P1",
        "unified action inbox frontend",
        "src/hooks/useInboxActions.ts",
        ("/api/inbox/actions", "useExecuteInboxAction"),
    ),
    GateCheck(
        "P0",
        "action inbox event audit table",
        "supabase/migrations/20260524_p0_action_events.sql",
        (
            "CREATE TABLE IF NOT EXISTS public.action_events",
            "p0_action_events_tenant_isolation",
        ),
    ),
    GateCheck(
        "P0",
        "action event recording API",
        "nexus_backend/app/routers/inbox.py",
        (
            "ActionEventRequest",
            '"/actions/{action_id}/events"',
            "action_events",
            "get_current_org_id",
        ),
    ),
    GateCheck(
        "P1",
        "action inbox feedback UI",
        "src/pages/InboxPage.tsx",
        ("useRecordInboxActionEvent", "handleActionEvent", "参考依据", "采纳", "忽略"),
    ),
    GateCheck(
        "P1",
        "mobile action-first home",
        "src/components/layout/MobileLayout.tsx",
        ("return <InboxPage />", "path === '/workbench'", "MobileWorkbenchPage"),
    ),
    GateCheck(
        "P1",
        "dashboard routes to growth command workspace",
        "src/routes/coreRoutes.tsx",
        (
            'path="dashboard" element={routeBoundary("Growth Command", <VMDCenter />)}',
            'path="inbox" element={routeBoundary("Inbox", <InboxPage />)}',
            'path="performance-dashboard"',
        ),
    ),
    GateCheck(
        "P1",
        "action inbox E2E mock",
        "e2e/fixtures/business-mocks.ts",
        ("**/api/inbox/actions**", "by_source"),
    ),
    GateCheck(
        "P1",
        "action inbox operating analytics API",
        "nexus_backend/app/routers/inbox.py",
        ('"/analytics"', "acceptance_rate", "stale_open_actions", "action_events"),
    ),
    GateCheck(
        "P1",
        "action inbox operating analytics UI",
        "src/pages/ActionAnalyticsPage.tsx",
        ("行动台运营分析", "采纳率", "高风险未闭环", "AI 复盘"),
    ),
    GateCheck(
        "P1",
        "five-view growth command navigation",
        "src/components/layout/Sidebar.tsx",
        (
            'label: "今日作战"',
            'label: "线索雷达"',
            'label: "客户与项目"',
            'label: "投标作战"',
            'label: "经营复盘"',
            "SPACE_MATCH_PREFIXES",
        ),
    ),
    GateCheck(
        "P1",
        "product space hubs",
        "src/pages/ProductSpaceHubPage.tsx",
        ("WorkspaceHubPage", "DataHubPage", "AICenterPage"),
    ),
    GateCheck(
        "P1",
        "proactive AI copilot",
        "src/components/ai/ProactiveCopilotPanel.tsx",
        ("useInboxActions(8)", "routeInsights", "让 AI 分析"),
    ),
    GateCheck(
        "P1",
        "embedded CRM AI layer",
        "src/pages/crm/CRMPage.tsx",
        ("CRMAIInsightLayer", "AIInsightPanel", "下一步客户动作", "生成跟进优先级"),
    ),
    GateCheck(
        "P1",
        "embedded contract AI risk layer",
        "src/pages/ContractManagement.tsx",
        ("ContractAIInsightLayer", "AIInsightPanel", "AI 合同风控摘要", "生成风险清单"),
    ),
    GateCheck(
        "P1",
        "embedded approval AI risk layer",
        "src/components/approval/ApprovalCenter.tsx",
        ("ApprovalAIRiskPanel", "AIInsightPanel", "下一步审批动作", "分析风险"),
    ),
    GateCheck(
        "P1",
        "embedded action inbox AI layer",
        "src/pages/InboxPage.tsx",
        (
            "ActionInboxInsightStrip",
            "AIInsightPanel",
            'surfaceId="inbox-priority"',
            "useInboxActions",
            "useExecuteInboxAction",
            "useRecordInboxActionEvent",
        ),
    ),
    GateCheck(
        "P1",
        "standard AI insight panel",
        "src/components/ai/AIInsightPanel.tsx",
        ("AITrustBadge", "evidence", "risks", "actions", "proactive-chat"),
    ),
    GateCheck(
        "P1",
        "AI insight action contract",
        "src/components/ai/AIInsightPanel.tsx",
        ("executeEndpoint", "requiresConfirmation", "ExperienceFeedback"),
    ),
    GateCheck(
        "P1",
        "experience feedback loop",
        "src/components/feedback/ExperienceFeedback.tsx",
        ("/api/feedback/experience", "nexus.experienceFeedback", "surface"),
    ),
    GateCheck(
        "P1",
        "role access guidance",
        "src/components/auth/RoleAccessHint.tsx",
        ("role-access-hint", "requiredRoles", "autoRedirectMs"),
    ),
    GateCheck(
        "P0",
        "product quality golden path E2E",
        "e2e/product-quality-golden-path.spec.ts",
        ("ai-insight-panel", "global-command-input", "boss-only workspace"),
    ),
    GateCheck(
        "P0",
        "frontend text quality scanner",
        "scripts/check_text_quality.mjs",
        ("mojibakePatterns", "TEXT_QUALITY_FILES", "placeholder copy"),
    ),
    GateCheck(
        "P1",
        "frontend quality package command",
        "package.json",
        ('"check:text"', '"quality:frontend"', "check_text_quality.mjs"),
    ),
    GateCheck(
        "P0",
        "unified work states",
        "src/components/common/WorkState.tsx",
        ("WorkEmptyState", "WorkErrorState", "WorkLoadingState"),
    ),
    GateCheck(
        "P2",
        "mobile action card stack",
        "src/components/mobile/MobileActionCardStack.tsx",
        ("审批速办", "客户跟进", "语音拜访速记", "右滑采纳"),
    ),
    GateCheck(
        "P3",
        "command bar executable actions and context",
        "src/components/layout/GlobalCommandBar.tsx",
        ("常用动作", "创建客户", "AI 当前上下文", "usePageContext"),
    ),
    GateCheck(
        "P0",
        "stream lifecycle split",
        "nexus_backend/app/agent/stream.py",
        (
            "stream_lifecycle",
            "emit_error_and_cleanup",
            "cleanup_on_disconnect",
            "ThinkTagTracker",
        ),
    ),
    GateCheck(
        "P1",
        "stream event helpers",
        "nexus_backend/app/agent/stream_events.py",
        ("process_stream_event", "ThinkTagTracker"),
    ),
    GateCheck(
        "P0",
        "gateway get_llm fail-closed",
        "nexus_backend/app/services/llm_gateway/__init__.py",
        ("requires resolved_config", "Use await resolve_model_config"),
    ),
    GateCheck(
        "P1",
        "standard health probes",
        "nexus_backend/app/main.py",
        ('"/health/live"', '"/health/ready"'),
    ),
    GateCheck(
        "P1",
        "web vitals backend route",
        "nexus_backend/app/routers/metrics.py",
        ("/web-vitals", "/slo"),
    ),
    GateCheck(
        "P2",
        "single theme context adapter",
        "src/contexts/ThemeContext.tsx",
        ("useEnhancedTheme", "return <>{children}</>"),
    ),
    GateCheck(
        "P2",
        "SLO dashboard route",
        "src/routes/adminRoutes.tsx",
        ('path="slo"', "SLODashboard"),
    ),
    GateCheck(
        "P2",
        "deployment readiness center",
        "src/pages/DeploymentReadinessPage.tsx",
        ("上线交付中心", "/api/system/deployment-health", "ACCEPTANCE_COMMANDS"),
    ),
    GateCheck(
        "P2",
        "deployment readiness route",
        "src/routes/adminRoutes.tsx",
        ('path="deployment-readiness"', "DeploymentReadinessPage"),
    ),
    GateCheck(
        "P2",
        "module tier governance",
        "src/config/featureFlags.ts",
        ("ModuleTier", "MODULE_TIERS", "getModuleTier", "getEnabledModulesByTier"),
    ),
    GateCheck(
        "P2",
        "mobile action gestures",
        "src/pages/InboxPage.tsx",
        ("handleSwipeEnd", "右滑采纳，左滑忽略"),
    ),
    GateCheck(
        "P2",
        "mobile AI voice memo trigger",
        "src/components/mobile/MobileAIFAB.tsx",
        ("onLongPress", "长按语音速记", "Mic"),
    ),
    GateCheck(
        "P2",
        "mobile workbench tier pruning",
        "src/components/mobile/MobileWorkbenchPage.tsx",
        ("moduleForPath", "MODULE_TIER_LABELS", "外部系统 / 低频入口"),
    ),
    GateCheck(
        "P2",
        "scientific instrument expert entry",
        "src/pages/ProductSpaceHubPage.tsx",
        ("IndustryExpertPanel", "科学仪器行业专家", "招投标评分"),
    ),
    GateCheck(
        "P2",
        "scientific instrument knowledge assets",
        "src/config/scientificInstrumentKnowledge.ts",
        (
            "SCIENTIFIC_INSTRUMENT_KNOWLEDGE_ASSETS",
            "Thermo Fisher LC/MS",
            "招投标评分拆解模板",
            "高校/科研院所采购决策链",
        ),
    ),
    GateCheck(
        "P2",
        "scientific instrument knowledge backend API",
        "nexus_backend/app/routers/industry_knowledge.py",
        (
            'prefix="/api/industry-knowledge"',
            "BUILTIN_ASSETS",
            "industry_knowledge_assets",
            "database",
            "builtin",
        ),
    ),
    GateCheck(
        "P2",
        "scientific instrument knowledge API hook",
        "src/hooks/useIndustryKnowledgeAssets.ts",
        (
            "api/industry-knowledge/assets",
            "frontend-fallback",
            "SCIENTIFIC_INSTRUMENT_ICON_BY_TYPE",
        ),
    ),
    GateCheck(
        "P2",
        "scientific instrument knowledge page",
        "src/pages/IndustryKnowledgePage.tsx",
        ("科学仪器行业知识资产", "用 AI 套用此资产", "资产化规则"),
    ),
    GateCheck(
        "P2",
        "action analytics management loop",
        "src/pages/ActionAnalyticsPage.tsx",
        ("行动趋势", "团队动作榜", "管理建议"),
    ),
    GateCheck(
        "P2",
        "CRM customer 360 detail",
        "src/pages/crm/CustomerDetailSheet.tsx",
        ("Customer360Panel", "客户 360 视图", "竞品态势", "报价 / 招投标"),
    ),
    GateCheck(
        "P2",
        "mobile native capture panel",
        "src/components/mobile/MobileNativeCapturePanel.tsx",
        ("语音速记", "拍名片", "附件归档", 'capture="environment"'),
    ),
    GateCheck(
        "P2",
        "module integration strategy",
        "src/config/featureFlags.ts",
        ("MODULE_INTEGRATION_STRATEGY", "third_party_first", "getIntegrationStrategy"),
    ),
    GateCheck(
        "P2",
        "first-week launch checklist",
        "src/components/product/LaunchChecklistPanel.tsx",
        ("首周落地任务", "nexus:first-week-launch-checklist"),
    ),
    GateCheck(
        "P2",
        "customer success dashboard",
        "src/pages/CustomerSuccessPage.tsx",
        ("客户成功看板", "首周激活目标"),
    ),
    GateCheck(
        "P2",
        "permission matrix page",
        "src/pages/PermissionMatrixPage.tsx",
        ("权限与 AI 安全矩阵", "Tool RBAC"),
    ),
    GateCheck(
        "P2",
        "AI operation transparency",
        "src/components/ai/genui/ReasoningTrace.tsx",
        ("AI 操作透明度", "HITL/RBAC"),
    ),
    GateCheck(
        "P0",
        "AI operating system product model",
        "src/config/aiOperatingSystem.ts",
        (
            "AI_OPERATING_CAPABILITIES",
            "Agent 仿真沙盒",
            "SOP → AOP 自然语言定义器",
            "CONTEXT_GRAPH_EDGES",
            "AUTONOMOUS_ACTION_POLICIES",
            "SEVEN_DAY_SUCCESS_PATH",
            "DEMO_WORKSPACE_ARTIFACTS",
            "ROLE_WORKBENCH_PROFILES",
        ),
    ),
    GateCheck(
        "P0",
        "AI operating system page",
        "src/pages/AIOperatingSystemPage.tsx",
        (
            "AI_OPERATING_CAPABILITIES",
            "AUTONOMOUS_ACTION_POLICIES",
            "useAIOperatingOverview",
            "useRunAgentSimulation",
            "useDefineAgentFromSop",
            "runSimulation",
            "defineAgent",
            'value="release"',
            'value="operations"',
            "P4-P6：产品形态与增长闭环",
            "AI-Native 场景",
            "行业 Agent 模板库",
        ),
    ),
    GateCheck(
        "P4",
        "AI operating system first-screen strip",
        "src/components/product/AIOperatingSystemStrip.tsx",
        (
            "助手工作台",
            "打开工作台",
            "营销场景",
            "助手管理",
            "业务知识",
        ),
    ),
    GateCheck(
        "P0",
        "AI operating system backend aggregation",
        "nexus_backend/app/routers/ai_operating_system.py",
        (
            "/api/ai-operating-system",
            '"/overview"',
            '"/simulate"',
            '"/define-agent"',
            "build_business_context_graph",
            "_simulate_messages",
            "_generate_agent_definition",
            "_value_summary",
            "_trust_summary",
        ),
    ),
    GateCheck(
        "P2",
        "ERPNext inspired enterprise foundation config",
        "src/config/erpnextInspiredOperatingModel.ts",
        (
            "ERP_NEXT_INSPIRED_CAPABILITIES",
            "DocType 元数据业务对象",
            "统一 Workflow/HITL 状态机",
            "不可变业务账本",
            "客户/供应商自助门户",
            "事件钩子与调度注册表",
            "报表/打印/导出模板",
            "模块 Onboarding 与 Demo Data",
            "迁移补丁治理",
        ),
    ),
    GateCheck(
        "P2",
        "ERPNext inspired enterprise foundation backend model",
        "nexus_backend/app/services/erpnext_inspired_operating_model.py",
        (
            "ERP_NEXT_INSPIRED_CAPABILITIES",
            "BUSINESS_OBJECT_BLUEPRINTS",
            "UNIFIED_WORKFLOW_BLUEPRINTS",
            "IMMUTABLE_LEDGER_STREAMS",
            "PORTAL_EXPERIENCE_BLUEPRINTS",
            "EVENT_HOOK_BLUEPRINTS",
            "REPORT_PRINT_BLUEPRINTS",
            "ONBOARDING_DEMO_PACKS",
            "MIGRATION_GOVERNANCE_RULES",
            "validate_erpnext_inspired_operating_model",
        ),
    ),
    GateCheck(
        "P2",
        "ERPNext inspired enterprise foundation API",
        "nexus_backend/app/routers/ai_operating_system.py",
        (
            '"/erpnext-inspired-model"',
            "get_erpnext_inspired_operating_model",
        ),
    ),
    GateCheck(
        "P2",
        "Odoo/Dolibarr inspired enterprise operating frontend model",
        "src/config/enterpriseOperatingFoundation.ts",
        (
            "NEXUS_EXECUTION_CONTEXT_BLUEPRINT",
            "deepseek-v4-flash",
            "BUSINESS_APP_MANIFESTS",
            "AI_SERVER_ACTIONS",
            "FIELD_PROMPT_POLICIES",
            "DOCUMENT_TEMPLATE_CENTER",
            "批量生成客户跟进计划",
            "never_send_to_llm",
            "客户 360 PDF",
        ),
    ),
    GateCheck(
        "P2",
        "Odoo/Dolibarr inspired enterprise operating backend model",
        "nexus_backend/app/services/enterprise_operating_foundation.py",
        (
            "NexusExecutionContext",
            "deepseek-v4-flash",
            "BUSINESS_APP_MANIFESTS",
            "AI_SERVER_ACTIONS",
            "FIELD_PROMPT_POLICIES",
            "DOCUMENT_TEMPLATE_CENTER",
            "validate_enterprise_operating_foundation",
        ),
    ),
    GateCheck(
        "P2",
        "Odoo/Dolibarr inspired enterprise operating API",
        "nexus_backend/app/routers/ai_operating_system.py",
        (
            '"/enterprise-operating-foundation"',
            "get_enterprise_operating_foundation",
        ),
    ),
    GateCheck(
        "P2",
        "Claude Code inspired core agent runtime frontend model",
        "src/config/coreAgentRuntimeV2.ts",
        (
            "AGENT_RUNTIME_LOOP_BLUEPRINT",
            "TOOL_LIFECYCLE_V2_STAGES",
            "DEFERRED_TOOL_SCHEMA_POLICY",
            "AGENT_RECOVERY_POLICIES",
            "PROMPT_SECTION_REGISTRY",
            "CONTEXT_COMPRESSION_PIPELINE",
            "PERMISSION_DECISION_V2_OUTCOMES",
            "SKILL_RUNTIME_MANIFESTS",
            "force_deepseek_v4_flash",
            "ToolSearch",
        ),
    ),
    GateCheck(
        "P2",
        "Claude Code inspired core agent runtime backend model",
        "nexus_backend/app/services/core_agent_runtime_v2.py",
        (
            "AgentRuntimeLoopState",
            "TOOL_LIFECYCLE_V2_STAGES",
            "DEFERRED_TOOL_SCHEMA_POLICY",
            "AGENT_RECOVERY_POLICIES",
            "PROMPT_SECTION_REGISTRY",
            "CONTEXT_COMPRESSION_PIPELINE",
            "PERMISSION_DECISION_V2_OUTCOMES",
            "SKILL_RUNTIME_MANIFESTS",
            "validate_core_agent_runtime_v2",
        ),
    ),
    GateCheck(
        "P2",
        "Claude Code inspired core agent runtime API",
        "nexus_backend/app/routers/ai_operating_system.py",
        (
            '"/core-agent-runtime-v2"',
            "get_core_agent_runtime_v2",
        ),
    ),
    GateCheck(
        "P2",
        "Agent operational hardening frontend model",
        "src/config/agentOperationalHardening.ts",
        (
            "AGENT_OPERATIONAL_HARDENING_AREAS",
            "MODEL_POLICY_ENFORCER",
            "deepseek-v4-flash",
            "gemini-3.1-pro-preview",
            "RUNTIME_V2_MAIN_CHAIN_ADOPTION",
            "TOOL_LIFECYCLE_V2_ROLLOUT",
            "DEFERRED_TOOL_SCHEMA_RUNTIME",
            "CONTEXT_COMPRESSION_EVAL_CASES",
            "SKILL_RUNTIME_ACTIVATION_RULES",
            "AGENT_REPLAY_BEHAVIOR_EVALS",
            "MEMORY_WRITE_GOVERNANCE",
            "PERMISSION_DECISION_EXPLAINABILITY",
            "AGENT_RUN_REPLAY_DEBUGGER_FIELDS",
        ),
    ),
    GateCheck(
        "P2",
        "Agent operational hardening backend model",
        "nexus_backend/app/services/agent_operational_hardening.py",
        (
            "AGENT_OPERATIONAL_HARDENING_AREAS",
            "MODEL_POLICY_ENFORCER",
            "LOW_COST_DEFAULT_MODEL",
            "EXPENSIVE_MODEL_DENYLIST",
            "RUNTIME_V2_MAIN_CHAIN_ADOPTION",
            "TOOL_LIFECYCLE_V2_ROLLOUT",
            "DEFERRED_TOOL_SCHEMA_RUNTIME",
            "CONTEXT_COMPRESSION_EVAL_CASES",
            "SKILL_RUNTIME_ACTIVATION_RULES",
            "AGENT_REPLAY_BEHAVIOR_EVALS",
            "MEMORY_WRITE_GOVERNANCE",
            "PERMISSION_DECISION_EXPLAINABILITY",
            "AGENT_RUN_REPLAY_DEBUGGER_FIELDS",
            "validate_agent_operational_hardening",
        ),
    ),
    GateCheck(
        "P2",
        "Agent operational hardening API",
        "nexus_backend/app/routers/ai_operating_system.py",
        (
            '"/agent-operational-hardening"',
            "get_agent_operational_hardening",
        ),
    ),
    GateCheck(
        "P1",
        "business graph context injection",
        "nexus_backend/app/agent/context_engine.py",
        (
            "BusinessGraphContextProvider",
            "business_context_graph",
            "context_engine.register(BusinessGraphContextProvider())",
        ),
    ),
    GateCheck(
        "P1",
        "business graph shared service",
        "nexus_backend/app/services/business_context_graph.py",
        (
            "GRAPH_QUERY_SPECS",
            "build_business_context_graph",
            "prompt_context",
            "客户项目",
        ),
    ),
    GateCheck(
        "P1",
        "versioned prompt registry",
        "nexus_backend/app/services/prompt_registry.py",
        ("PromptManifest", "prompt_version", "eval_gates", "prompt_registry"),
    ),
    GateCheck(
        "P1",
        "context quality evidence pack",
        "nexus_backend/app/services/context_quality.py",
        (
            "ContextQualityScore",
            "quality_score",
            "permission_scope",
            "build_evidence_pack",
        ),
    ),
    GateCheck(
        "P1",
        "agent CI replay harness",
        "nexus_backend/app/services/agent_ci_service.py",
        (
            "AgentCIService",
            "DEFAULT_AGENT_CI_CASES",
            "behavior_diff",
            "agent_replay_harness.evaluate_trace",
        ),
    ),
    GateCheck(
        "P2",
        "Hermes-style improvement proposals",
        "nexus_backend/app/services/agent_improvement_service.py",
        ("AgentImprovementService", "self_mutation_allowed", "人工批准", "小流量灰度"),
    ),
    GateCheck(
        "P2",
        "memory hygiene service",
        "nexus_backend/app/services/memory_hygiene_service.py",
        ("MemoryHygieneService", "hygiene_score", "golden_example_target"),
    ),
    GateCheck(
        "P2",
        "agent improvement center page",
        "src/pages/AgentImprovementCenterPage.tsx",
        (
            "usePromptRegistry",
            "useMemoryHygiene",
            "useAgentImprovementProposals",
            "AgentOpsQuality",
            "AgentOpsReleases",
        ),
    ),
    GateCheck(
        "P2",
        "agent evolution APIs",
        "nexus_backend/app/routers/ai_operating_system.py",
        (
            '"/prompt-registry"',
            '"/agent-ci"',
            '"/improvement-proposals"',
            '"/memory-hygiene"',
        ),
    ),
    GateCheck(
        "P2",
        "agent evolution database persistence",
        "supabase/migrations/20260525_agent_evolution_ops.sql",
        (
            "agent_prompt_versions",
            "agent_improvement_proposals",
            "agent_eval_cases is intentionally not created here",
            "agent_reward_events",
            "agent_skill_marketplace",
            "agent_redteam_findings",
            "agent_trust_reports",
        ),
    ),
    GateCheck(
        "P2",
        "agent eval cases schema reconciliation",
        "supabase/migrations/20260525_agent_eval_cases_schema_reconcile.sql",
        (
            "ADD COLUMN IF NOT EXISTS organization_id",
            "column_name = 'org_id'",
            "column_name = 'criticality'",
            "column_name = 'query'",
            "agent_eval_cases_tenant_read",
        ),
    ),
    GateCheck(
        "P2",
        "agent evolution operations service",
        "nexus_backend/app/services/agent_evolution_ops_service.py",
        (
            "AgentEvolutionOpsService",
            "AGENT_SKILL_CATALOG",
            "MULTI_AGENT_PROTOCOL",
            "REDTEAM_SCENARIOS",
            "build_low_quality_queue",
            "build_reward_model",
            "build_trust_center_report",
        ),
    ),
    GateCheck(
        "P2",
        "agent evolution operations API",
        "nexus_backend/app/routers/ai_operating_system.py",
        (
            '"/evolution-ops"',
            '"/proposals/{proposal_key}/decision"',
            "AgentProposalDecisionRequest",
        ),
    ),
    GateCheck(
        "P2",
        "agent evolution operations UI",
        "src/pages/AgentImprovementCenterPage.tsx",
        (
            "useAgentEvolutionOps",
            "AgentOpsOverview",
            "AgentOpsQuality",
            "AgentOpsReleases",
            "AgentOpsRuntime",
            "decideProposal",
            "gray_release",
            "rollback",
        ),
    ),
    GateCheck(
        "P2",
        "agent evolution operations hooks",
        "src/hooks/useAIOperatingSystem.ts",
        ("useAgentEvolutionOps", "useDecideAgentProposal", "AgentEvolutionOpsResult"),
    ),
    GateCheck(
        "P2",
        "last-mile delivery hardening",
        "scripts/run_last_mile_checks.ps1",
        (
            "RealBackend",
            "RealMigrations",
            "verify_migration_replay.py",
        ),
    ),
    GateCheck(
        "P2",
        "memory-safe frontend build",
        "scripts/build_frontend.mjs",
        (
            "--max-old-space-size",
            "VITE_BUILD_PROFILE",
            "vite",
        ),
    ),
    GateCheck(
        "P2",
        "large agent eval dataset",
        "nexus_backend/tests/production_proof/fixtures/agent_eval_cases_200.json",
        (
            "agent-eval-200",
            "vmd_campaign",
            "respects_tenant_context",
        ),
    ),
    GateCheck(
        "P2",
        "tool failure attribution",
        "nexus_backend/app/agent/tool_failure_attribution.py",
        (
            "invalid_params",
            "permission_denied",
            "network_error",
        ),
    ),
    GateCheck(
        "P2",
        "AI behavior weekly report",
        "src/hooks/useAIWeeklyReport.ts",
        (
            "ai-weekly-report",
            "human_overrides",
            "estimated_hours_saved",
        ),
    ),
    GateCheck(
        "P2",
        "module convergence policy",
        "src/config/featureFlags.ts",
        (
            "MODULE_FOCUS_POLICY",
            "THIRD_PARTY_FIRST_MODULES",
            "isThirdPartyFirstModule",
        ),
    ),
    GateCheck(
        "P2",
        "schema convergence and real router eval",
        "scripts/audit_schema_convergence.py",
        (
            "SCHEMA_CONVERGENCE_OK",
            "agent_heartbeat_runs",
            "organization_id",
        ),
    ),
    GateCheck(
        "P2",
        "agent eval baseline service",
        "nexus_backend/app/services/agent_eval_baseline_service.py",
        (
            "_intent_for",
            "run_router_baseline",
            "accuracy",
        ),
    ),
    GateCheck(
        "P2",
        "Aeon-inspired Agent Ops runtime",
        "nexus_backend/app/services/agent_ops_runtime_service.py",
        (
            "build_heartbeat",
            "build_skill_health",
            "build_reactive_triggers",
            "build_self_repair",
            "build_skill_chains",
            "build_universal_var",
            "build_operating_memory",
            "build_instance_fleet",
            "build_persona_soul",
            "build_external_capabilities",
            "persist_dashboard",
            "register_heartbeat_schedule",
            "trigger_actions",
        ),
    ),
    GateCheck(
        "P2",
        "Aeon-inspired Agent Ops UI and API",
        "src/pages/AgentImprovementCenterPage.tsx",
        (
            "AgentOpsRuntime",
            'value="runtime"',
            "useRunAeonInspiredHeartbeat",
            "useRegisterAeonHeartbeatSchedule",
            "onRunHeartbeat",
            "onRegisterSchedule",
        ),
    ),
    GateCheck(
        "P2",
        "Agent Ops system actions in inbox",
        "nexus_backend/app/routers/inbox.py",
        ("_load_system_actions", "Reactive trigger fired", '.eq("source", "system")'),
    ),
    GateCheck(
        "P5",
        "AI operating system route and E2E",
        "e2e/customer-business-acceptance.spec.ts",
        (
            "/ai-operating-system",
            "AI 作战室",
            "AI 价值与信任仪表盘",
            "Agent 仿真沙盒",
            "生成 Agent 定义",
            "科学仪器 Demo 空间",
        ),
    ),
    GateCheck(
        "P0",
        "small-company launch profile",
        "src/config/featureFlags.ts",
        (
            "SMALL_COMPANY_LAUNCH_MODULES",
            "VITE_LAUNCH_PROFILE",
            "EXTENDED_LAUNCH_MODULES",
        ),
    ),
    GateCheck(
        "P0",
        "customer acceptance gate",
        "scripts/customer_acceptance_gate.py",
        ("SMALL_COMPANY_MODULES", "Tool RBAC", "Irreversible HITL"),
    ),
    GateCheck(
        "P0",
        "customer business acceptance E2E",
        "e2e/customer-business-acceptance.spec.ts",
        (
            "CRM can create a customer",
            "AI chat sends a message",
            "employee role is blocked",
            "golden path covers action inbox",
            "/action-analytics",
            "/industry-knowledge",
        ),
    ),
    GateCheck(
        "P1",
        "customer handoff generator",
        "scripts/generate_customer_handoff.py",
        ("customer-handoff.md", "Required Acceptance Commands", "small_company"),
    ),
    GateCheck(
        "P1",
        "customer acceptance criteria",
        "docs/CUSTOMER_ACCEPTANCE_CRITERIA.md",
        (
            "Default Launch Profile",
            "Acceptance Rules",
            "customer-business-acceptance.spec.ts",
        ),
    ),
    GateCheck(
        "P0",
        "production proof gate",
        "scripts/production_proof_gate.py",
        (
            "five golden business flows",
            "agent graph e2e contract",
            "tenant RLS isolation contract",
            "LLM VCR replay cassette",
        ),
    ),
    GateCheck(
        "P0",
        "production proof test suite",
        "nexus_backend/tests/production_proof/test_golden_business_flows.py",
        ("RUN_REAL_GOLDEN_FLOWS", "golden-login-crm-ai-approval-close"),
    ),
    GateCheck(
        "P0",
        "production proof CI wiring",
        ".github/workflows/ci.yml",
        ("production_proof_gate.py", "pytest tests/production_proof"),
    ),
]


def main() -> int:
    failures: list[str] = []
    warnings: list[str] = []
    print("Release quality gate: P0-P6 static controls")
    for check in CHECKS:
        ok, reason = run_check(check)
        status = "OK" if ok else ("WARN" if check.severity == "warning" else "FAIL")
        print(f"{status:<4} [{check.level}] {check.name}")
        if ok:
            continue
        message = f"[{check.level}] {check.name}: {reason}"
        if check.severity == "warning":
            warnings.append(message)
        else:
            failures.append(message)

    print("")
    print(f"Summary: {len(failures)} critical failure(s), {len(warnings)} warning(s).")
    if failures:
        for item in failures:
            print(f" - {item}")
        return 1
    return 0


if __name__ == "__main__":
    sys.exit(main())
