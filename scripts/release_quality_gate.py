...
CHECKS = [
    ...
    GateCheck("P1", "API public route auth matrix", "nexus_backend/app/core/api_auth_matrix.py", ("PUBLIC_API_ROUTE_REASONS", "/api/metrics/web-vitals", "/api/oauth/token")),
    GateCheck("P1", "API auth matrix regression test", "nexus_backend/tests/unit/test_api_auth_matrix.py", ("test_every_dependency_free_api_route_is_explicitly_allowlisted", "test_public_api_route_allowlist_has_no_stale_entries")),
    GateCheck("P1", "tenant scoped DB dependency", "nexus_backend/app/core/dependencies.py", ("def get_request_db", "request.state", "status_code=401")),
    GateCheck("P1", "ASGI unhandled exception boundary", "nexus_backend/app/core/security_middleware.py", ("class UnhandledExceptionMiddleware", "Unhandled request exception")),
    GateCheck("P1", "unhandled exception boundary wiring", "nexus_backend/app/startup/middleware.py", ("UnhandledExceptionMiddleware", "add_middleware")),
    GateCheck("P1", "tenant org auth regression test", "nexus_backend/tests/unit/test_auth_org_context.py", ("test_missing_org_without_auth_is_401", "test_missing_org_after_auth_failure_is_401")),
    GateCheck("P1", "prompt firewall fast-path regression test", "nexus_backend/tests/unit/test_prompt_firewall_fast_path.py", ("test_context_overflow_does_not_call_llm_judge", "context_overflow")),
    GateCheck("P1", "customer acceptance mock auth contract", "e2e/customer-business-acceptance.spec.ts", ("mockLoggedInState(page, role)", "getByTestId('chat-input')", "toHaveURL(/\\/dashboard/")),
