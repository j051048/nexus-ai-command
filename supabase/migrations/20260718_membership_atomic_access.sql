-- Atomic and idempotent organization membership changes.
--
-- The application previously updated subscriptions, organizations and the
-- access-version ledger in separate PostgREST requests. A network failure
-- between those calls could leave entitlement state partially applied. This
-- RPC keeps the authoritative state and its rollback evidence in one database
-- transaction. Reusing p_change_id with the same payload is a safe replay.
...
BEGIN
  ...
  IF FOUND THEN
    IF existing_change.org_id <> p_org_id
       OR existing_change.next_snapshot IS DISTINCT FROM next_snapshot THEN
...
COMMENT ON FUNCTION public.set_subscription_access_atomic(
  UUID, TEXT, TEXT, TEXT, TIMESTAMPTZ, TEXT, TEXT, TEXT
) IS 'Atomically applies one organization entitlement and records its rollback evidence; p_change_id is the idempotency key.';
