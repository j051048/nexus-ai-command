...
serve(async (req) => {
  ...
  try {
    ...
    const ZHZ_TECH_API_KEY = Deno.env.get("ZHZ_TECH_API_KEY");

    if (!ZHZ_TECH_API_KEY) {
      throw new Error("ZHZ_TECH_API_KEY is not configured");
    }
    ...
    const response = await fetch("https://ai.gateway.zhz-tech.dev/v1/chat/completions", {
      method: "POST",
      headers: {
        Authorization: `Bearer ${ZHZ_TECH_API_KEY}`,
