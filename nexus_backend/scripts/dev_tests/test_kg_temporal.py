...
async def test_temporal_validity():
    ...
    try:
        res = await supabase.table("organizations").select("id").limit(1).execute()
        if res.data:
    ...
    first_res = await query_entity_relations(org_id, entity_name, db=supabase, include_historical=True)
    if not first_res:
    ...
    current = await query_entity_relations(org_id, entity_name, db=supabase, include_historical=False)
    print(f"Current relations count: {len(current)}")
    for r in current:
        print(f"  {r['source_entity']} -{r['relationship']}-> {r['destination_entity']} (valid_to: {r['valid_to']})")
    
    # 4. Verify historical state (should see both A and B, A should have valid_to set)
    print("\n4. Verifying historical state...")
    all_history = await query_entity_relations(org_id, entity_name, db=supabase, include_historical=True)
    print(f"All history count: {len(all_history)}")
    for r in all_history:
        status = "HISTORICAL" if r.get("valid_to") else "CURRENT"
        print(f"  [{status}] {r['source_entity']} -{r['relationship']}-> {r['destination_entity']} [From: {r['valid_from']} To: {r['valid_to']}]")
    ...
    for r in at_time:
        print(f"  {r['source_entity']} -{r['relationship']}-> {r['destination_entity']}")
