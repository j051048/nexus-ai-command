...
from app.agent.context_compiler import ContextBudgetExceeded, ContextCompilePolicy, ContextCompiler, context_message
...
from app.services.conversation_memory.governance import filter_current_memories, memory_is_current
...
def compiler(monkeypatch):
    monkeypatch.setattr(ContextCompiler, "_estimate_tokens", staticmethod(lambda content, model="": len(content)))
    return ContextCompiler()


def test_required_blocks_fail_closed_instead_of_dropping(compiler):
    with pytest.raises(ContextBudgetExceeded):
        compiler.compile([SystemMessage(content="a" * 60), SystemMessage(content="b" * 60)],
                         policy=ContextCompilePolicy(max_input_tokens=100, reserved_output_tokens=0, reserved_history_tokens=0))
...
def test_evidence_cannot_promote_itself_to_policy(compiler):
    ...
    compiled, report = compiler.compile([evidence, required], policy=ContextCompilePolicy(max_input_tokens=100, reserved_output_tokens=0, reserved_history_tokens=0))
...
def test_actual_history_and_tools_are_counted(compiler):
    with pytest.raises(ContextBudgetExceeded):
        compiler.compile([SystemMessage(content="policy"), HumanMessage(content="x" * 120)], policy=ContextCompilePolicy(max_input_tokens=120, reserved_history_tokens=0, reserved_output_tokens=0, reserved_tool_tokens=20))
...
def test_document_requires_explicit_tenant_and_current_lifecycle():
    assert document_access_reason({}, user_id="u", organization_id="a") == "not_accessible"
    assert document_access_reason({"organization_id": "a", "revoked_at": "today"}, user_id="u", organization_id="a") == "not_current"
...
@pytest.mark.parametrize("patch", [{"expires_at": "2000-01-01"}, {"valid_until": "bad"}, {"superseded_by": "new"}, {"lifecycle_state": "archived"}, {"category": "company_policy"}])
...
def test_retrieval_filters_foreign_and_expired_memory():
    good = {"id": "ok", "organization_id": "a", "user_id": "u", "lifecycle_state": "confirmed"}
    rows = [good, {**good, "organization_id": "b"}, {**good, "expires_at": "2000-01-01"}]
    assert filter_current_memories(rows, user_id="u", org_id="a") == [good]


def test_explicit_statement_is_not_company_policy_approval():
    decision = evaluate_memory_admission(value="All discounts are approved", category="company_policy", confidence=1, source="user_explicit", extraction_method="user_explicit", metadata={}, valid_until=None, evidence_ref="doc")
