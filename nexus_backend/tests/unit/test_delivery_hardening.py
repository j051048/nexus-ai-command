...
from app.services.delivery_requirement_service import (
    evaluate_delivery_requirements,
...
@pytest.mark.parametrize(
    "payload",
    [
        {"catalog_quantities": {"FD-F": 0}},
        {"catalog_quantities": {"FD-F": True}},
        {"catalog_quantities": {"FD-F": 10001}},
        {"budget": -1},
        {"budget": "NaN"},
        {"forbidden_claims": [" "]},
        {"unit_price": 1},
...
def test_fact_needs_source_and_local_citation():
    ...
    assert (
        check("提供安装培训 [EVID:doc:c1] 永久免费保修")["findings"][0]["code"]
...
async def test_quote_uses_tenant_price_book_and_tax_not_model_prices():
    ...
    assert (
        evaluate_delivery_requirements(
            "",
            ArtifactSpec(delivery_requirements=requirements, commercial_quote=quote),
            {},
        )["findings"][0]["code"]
...
def test_long_document_retrieval_keeps_late_terms_and_complete_passages():
    ...
    record = EvidenceRecord(
        document_id="d", chunk_id="c", title="服务", excerpt=excerpt, purposes=["售后"]
    )
...
async def test_stage_resume_invalidation_scope_and_errors():
    db = CheckpointDB()
    scope = dict(
        db=db, organization_id="org", user_id="user", job_id="job", lease_token="lease"
    )
    ...
    operation = AsyncMock(
        return_value=(
            {"sections": ["原稿"]},
            SimpleNamespace(
                usage={"call_cost": 0.001},
                finish_reason="stop",
                model_code="fixed",
                raw_response={"secret": "never-store"},
            ),
        )
    )
    ...
    assert response.usage["call_cost"] == 0.001 and value["sections"] == ["原稿"]
    assert "secret" not in str(db.saved)
    await ArtifactStageCache(**scope, context={"evidence": "v2"}).pair(
        "analysis", operation, request="方案"
    )
    await ArtifactStageCache(**{**scope, "user_id": "other"}).pair(
        "analysis", operation, request="方案"
    )
...
async def test_incomplete_stage_is_not_cached(finish):
    db = CheckpointDB()
    cache = ArtifactStageCache(
        db=db, organization_id="o", user_id="u", job_id="j", lease_token="l"
    )
    await cache.pair(
        "section",
        AsyncMock(return_value=(["partial"], SimpleNamespace(finish_reason=finish))),
    )
    assert not db.saved
...
def test_checkpoint_migration_has_actor_lease_and_service_guards():
    sql = (
        ...
        / "supabase/migrations/20260909_001_artifact_stage_checkpoints.sql"
    ).read_text()
    for token in (
        "SECURITY INVOKER",
...
async def test_preview_and_revision_recheck_source_acl_and_version():
    ...
    version = {
        "evidence_snapshot": {
            "records": [{"document_id": doc_id, "source_version": "v1"}]
        }
    }
    row = {
        "id": doc_id,
...
def test_revision_keeps_requirements_and_resets_approval():
    payload = revision_payload(
        {
            ...
            "metadata": {
                "content_contract": {
                    ...
                    "delivery_requirements": {"budget": 200},
                }
            },
        },
        {},
    ...
    )  # Worker reads authoritative, permission-checked source.


def test_cost_decimals_and_unknown_cost_are_preserved():
    assert merge_delivery_usage({"call_cost": 0.0003}, {"call_cost": 0.0004})[
        "call_cost"
