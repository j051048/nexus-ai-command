...
from app.services.artifact_docx_renderer import render_artifact_docx, render_artifact_xlsx
...
def test_versioned_synthetic_exports_are_not_customer_proof(tmp_path, case):
    ...
    path.write_bytes(render_artifact_docx({"title": title, "content_markdown": f"# {title}\n\n## 客户需求\n" + paragraph * 20}, {}))
    manifest = {"cases": [{**case, "file": path.name, "title": title}],
                "provenance": dict(git_commit="test", pipeline_version="test", model="synthetic-no-model",
                                   template_version="test", dataset_version="synthetic.v1")}
...
def test_formula_like_prose_is_never_executed():
    data = render_artifact_xlsx({"title": "=1+1", "content_markdown": "=HYPERLINK(\"https://example.invalid\",\"link\")"}, {})
    ...
    workbook.close()
    assert "unexpected_executable_formula" in validate_export(buffer.getvalue(), "xlsx")["errors"]
...
def test_release_requires_criteria_files_and_provenance(tmp_path):
    ...
    assert not evaluate_release({"cases": [{"id": "1", "file": "../secret.docx"}]}, tmp_path)["ok"]


def test_economics_deduplicates_and_never_invents_cost():
    generated = {"artifact_id": "1", "event_type": "generated", "metadata": {"usage": {"call_cost": .004}}}
    adopted = {"artifact_id": "1", "event_type": "used", "created_at": "1", "metadata": {"rework_minutes": 5}}
    report = artifact_value_metrics([generated, generated, adopted, adopted])
    assert report["generated_artifacts"] == 1
    assert report["retained_cost_per_adopted_artifact_usd"] == .004
    assert report["mean_reported_rework_minutes"] == 5
    assert artifact_value_metrics([adopted])["retained_cost_per_adopted_artifact_usd"] is None
    assert artifact_value_metrics([generated, adopted, {**adopted, "event_type": "discarded", "created_at": "2"}])["latest_adopted_artifacts"] == 0
