...
@pytest.mark.asyncio
@respx.mock
async def test_openai_429_rate_limit_fallback():
    """
    ...
    openai_route.mock(return_value=httpx.Response(429, json={"error": {"message": "Rate limit reached."}}))
    ...
    assert response.status_code != 500, f"Expected graceful degraded response, got 500 internal server crash. Text: {response.text}"
    ...
    assert response.status_code in [200, 429, 503, 504], f"Unexpected status code under 429 condition: {response.status_code}"
...
@pytest.mark.asyncio
@respx.mock
async def test_llm_timeout_circuit_breaker():
    """
    Integration & API Test (Fault Injection):
    ...
    if response.status_code == 200:
        data = response.json()
        assert "timeout" in str(data).lower() or "later" in str(data).lower() or "error" in str(data).lower(), "Expected degradation message in 200 response"
