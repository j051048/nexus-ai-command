...
class TestLoopDetectionAndSafety:
    ...
    async def test_after_execute_loop_second_time(self):
        import asyncio
        ...
        with patch("app.agent.graph._detect_loop", return_value=True), patch(
            "app.agent.graph.get_completed_tools", return_value=[mock_tc]
        ), patch("app.agent.graph._has_irreversible_tool", return_value=True), patch(
            "app.services.failure_log_service.failure_log_service.log_failure",
...
class TestFastSynthesis:
    def test_after_execute_all_tools_success_fast_synthesis(self):
        ...
        state = _make_state(complexity="MODERATE", completed_tool_calls=[mock_tc])
        with patch(
            "app.agent.graph.get_completed_tools",
            return_value=state["completed_tool_calls"],
    ...
    def test_after_execute_irreversible_unconfirmed_goes_to_reflect(self):
        ...
        state = _make_state(completed_tool_calls=[mock_tc])
