...
def test_fuzz_ai_assistant_endpoint(payload):
    ...
    try:
        ...
        assert response.status_code < 500, f"Fuzzing caused internal server error! Payload: {json.dumps(payload)}"
...
@given(
    query=st.text(min_size=1000, max_size=50000) # Testing massive strings
)
def test_fuzz_massive_prompt(query):
    ...
    assert response.status_code < 500, f"Fuzzing massive prompt caused internal server error!"
