...
_FRIENDLY_MESSAGES: list[tuple[re.Pattern, str, str | None]] = [
    ...
    (
        re.compile(
            r"permission|forbidden|403|权限不足|access.*denied|unauthorized|401", re.I
        ),
        "您没有执行此操作的权限。",
        "请联系管理员获取相应权限。",
    ),
    ...
    (
        re.compile(
            r"not\s*found|404|找不到|no.*rows|does\s*not\s*exist|PGRST116", re.I
        ),
        "未找到相关记录，可能已被删除或不存在。",
        "要我帮您重新搜索吗？",
    ),
    ...
    (
        re.compile(
            r"column.*not.*exist|PGRST204|undefined.*column|未知.*列|字段.*不存在", re.I
        ),
        "数据结构有变化，请联系管理员检查。",
        None,
    ),
    ...
    (
        re.compile(
            r"type.*mismatch|invalid.*input|格式.*错误|invalid.*value|check.*constraint",
            re.I,
        ),
        "输入的数据格式不正确，请检查后重试。",
        None,
    ...
    (
        re.compile(
            r"connection.*(?:refused|reset|error)|network|ECONNREFUSED|网络", re.I
        ),
        "网络连接异常，请稍后再试。",
        None,
    ),
