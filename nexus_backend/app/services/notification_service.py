...
def _auto_register_adapters():
    ...
    if settings.SMTP_HOST and settings.SMTP_USER:
        try:
            from app.services.notification_adapters.email_adapter import (
    ...
    if getattr(settings, "WECOM_WEBHOOK_URL", None):
        try:
            from app.services.notification_adapters.wecom_adapter import (
    ...
    if getattr(settings, "DINGTALK_WEBHOOK_URL", None):
        try:
            from app.services.notification_adapters.dingtalk_adapter import (
    ...
    if getattr(settings, "FEISHU_WEBHOOK_URL", None):
        try:
            from app.services.notification_adapters.feishu_adapter import (
