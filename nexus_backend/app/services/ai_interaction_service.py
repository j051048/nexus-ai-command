...
class AIInteractionService:
    ...
    SUGGESTION_TRIGGERS = {
        "idle_timeout": {
            "delay_seconds": 30,
            "suggestions": ["需要我帮您分析数据吗？", "我可以帮您查询最新的业务报告", "有什么问题想问我吗？"],
        },
        "page_load": {"suggestions": ["我可以帮您处理这些数据", "点击这里开始智能分析"]},
        "error_occurred": {"suggestions": ["看起来遇到了问题，需要帮助吗？", "我可以帮您排查这个错误"]},
        "task_completed": {"suggestions": ["任务已完成，还需要其他帮助吗？", "我可以帮您生成报告"]},
    }

    # Smart completion patterns
    COMPLETION_PATTERNS = [
        {"pattern": r"查询(.+)", "completions": ["销售数据", "用户信息", "订单记录", "库存状态"]},
        {"pattern": r"分析(.+)", "completions": ["销售趋势", "用户行为", "产品表现", "市场数据"]},
        {"pattern": r"生成(.+)", "completions": ["报告", "图表", "摘要", "建议"]},
        {"pattern": r"帮我(.+)", "completions": ["写一封邮件", "整理数据", "创建任务", "安排会议"]},
    ]

    # Conversation starters based on context
    CONVERSATION_STARTERS = {
        "dashboard": ["今天的数据表现如何？", "帮我分析关键指标变化", "生成今日业务摘要"],
        "documents": ["帮我搜索相关文档", "总结这个文档的要点", "找出文档中的关键信息"],
        "settings": ["推荐最佳配置方案", "帮我优化系统设置"],
        "reports": ["生成最新的业务报告", "分析报告数据趋势", "对比历史数据"],
        "default": ["有什么我可以帮您的？", "您可以问我任何问题", "需要我帮您做什么？"],
    }
    ...
    async def get_proactive_suggestion(
        self, user_id: str, trigger: str, context: dict[str, Any] = None
    ) -> AIInteraction | None:
        """
    ...
    async def get_smart_completions(self, user_id: str, current_input: str, limit: int = 5) -> list[str]:
        """
        ...
        for pattern_config in self.COMPLETION_PATTERNS:
            if re.search(pattern_config["pattern"], current_input):
                for completion in pattern_config["completions"]:
                    full_completion = re.sub(pattern_config["pattern"], f"\\1{completion}", current_input)
    ...
    async def get_contextual_help(
        self, user_id: str, current_page: str, current_action: str = None
    ) -> AIInteraction | None:
        """
    ...
    def _generate_contextual_help(self, page: str, action: str = None) -> dict | None:
        ...
        help_map = {
            ...
            "report_creator": {"message": "需要帮您生成报告吗？可以说出您的需求", "action": "start_report_wizard"},
            "data_import": {"message": "上传数据文件后，我可以自动分析并提供建议", "action": "show_import_tips"},
            "settings": {"message": "不确定如何配置？让我帮您优化设置", "action": "optimize_settings"},
        }

        return help_map.get(page)

    async def get_predictive_actions(self, user_id: str, context: dict[str, Any]) -> list[AIInteraction]:
        """
    ...
    async def get_conversation_starters(self, user_id: str, context: str = "default") -> list[str]:
        """
    ...
    def record_interaction(self, user_id: str, interaction_type: str, input_text: str, action_taken: str = None):
    ...
    def _prioritize_suggestions(self, suggestions: list[str], preferences: dict) -> list[str]:
