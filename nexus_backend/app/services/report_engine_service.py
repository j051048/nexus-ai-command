...
def _is_safe_sql(sql: str) -> bool:
    ...
    return not any(kw in sql_lower for kw in dangerous) and sql_lower.startswith(
        "select"
    )
...
async def generate_sql(nl_query: str, org_id: str) -> dict[str, Any]:
    ...
    try:
        ...
        config = await resolve_model_config(
            scene_code="report_engine", complexity_tier="balanced"
        )
...
async def generate_insight(nl_query: str, data: list) -> str:
    ...
    try:
        ...
        config = await resolve_model_config(
            scene_code="report_engine", complexity_tier="light"
        )
        llm = get_langchain_llm_sync(**config)
...
async def list_saved_reports(org_id: str, user_id: str, limit: int = 20) -> list:
    ...
    # Own reports
    own = (
        await supabase.table("saved_reports")
        .select("id, title, nl_query, summary, is_public, created_at")
...
async def list_schedules(org_id: str) -> list:
    ...
    res = (
        await supabase.table("report_schedules")
        .select(
        ...
        .update(
            {
                "is_active": active,
                "next_execution_at": (
                    _compute_next_execution_after_toggle(active).isoformat()
                    if active
                    else None
                ),
            }
...
async def execute_scheduled_report(schedule_id: str) -> dict[str, Any]:
    ...
    next_exec = _compute_next_execution(
        sched["schedule_type"],
        sched["hour"],
        sched.get("day_of_week", 1),
        sched.get("day_of_month", 1),
    )
    ...
    # Push to recipients
    await _push_to_recipients(
        sched.get("recipients", []), sched["name"], summary, report.get("id")
    )
...
async def _push_to_recipients(
    recipients: list, name: str, summary: str, report_id: str | None
):
...
def _compute_next_execution(
    schedule_type: str, hour: int, day_of_week: int = 1, day_of_month: int = 1
) -> datetime:
