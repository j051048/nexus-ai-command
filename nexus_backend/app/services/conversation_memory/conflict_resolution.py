...
2. UPDATE — 同一主题但信息发生变化:
   ...
   ⚠️ UPDATE合并策略（重要）：
   - 如果新旧信息是"扩展关系"（新包含旧） → merged_value = 新信息
   - 如果新旧信息是"补充关系"（各有侧重） → merged_value = "旧信息 + 新信息"
   - 如果新旧信息是"替换关系"（时间/状态变化） → merged_value = 新信息，并更新valid_from
   - 示例：旧="喜欢打球" + 新="喜欢和朋友打球" → merged="喜欢和朋友打球"（扩展）
   - 示例：旧="负责华东区" + 新="负责华南区" → merged="负责华东区和华南区"（补充）
