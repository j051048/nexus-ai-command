...
RULE_CATEGORIES = frozenset(
    {"policy", "business_rule", "company_policy", "pricing_policy", "service_policy"}
)


def memory_is_current(memory: dict[str, Any], *, now: datetime | None = None) -> bool:
    now = now or datetime.now(UTC)
    if memory.get("superseded_by") or memory.get("lifecycle_state", "active") not in {
        "active",
        "confirmed",
    }:
        return False
...
def filter_current_memories(
    ...
    *,
    ...
    return [
        ...
        if memory_is_current(memory)
        and can_access_memory(
            memory.get("visibility") or "private",
            str(memory.get("user_id") or ""),
            memory.get("organization_id"),
            user_id,
            org_id,
            user_role,
        )
