...
GUARDED_HANDLERS = frozenset({
    "calculate_deal_bonus", "auto_create_contract_from_deal",
    "auto_create_invoice_from_contract", "update_sales_metrics_on_payment",
})


async def dispatch_business_event(handler: Any, event: Any) -> None:
    if handler.__module__ != "app.services.event_bus" or handler.__name__ not in GUARDED_HANDLERS:
        await handler(event)
        return
    ...
    actor = await (supabase.table("users").select("id,status")
                   .eq("id", actor_id).eq("organization_id", org_id)
                   .maybe_single().execute())
    if not actor.data or actor.data.get("status") in {"inactive", "disabled", "deleted", "suspended"}:
    ...
    params = {"p_organization_id": org_id, "p_event_id": event.id,
              "p_handler": handler.__name__, "p_actor_id": actor_id,
    ...
    try:
        ...
        await (supabase.table("business_event_receipts")
               .update({"status": "needs_attention"})
               .eq("organization_id", org_id).eq("event_id", event.id)
               .eq("handler", handler.__name__).eq("status", "processing").execute())
        raise
    await (supabase.table("business_event_receipts")
