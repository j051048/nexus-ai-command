...
async def get_usage_stats(req: Request, user_id: str = Depends(get_current_user_id)):
    ...
    try:
        ...
        rows = result.data or []
        total_tokens = sum(
            r.get("total_input_tokens", 0) + r.get("total_output_tokens", 0) for r in rows
        )
        return api_success(data={
            "recent_usage": rows,
    ...
    try:
        ...
        if not db or not org_id:
            return api_success(data={
                ...
                "total_requests": 0, "period_days": days,
            })

        since = (date.today() - timedelta(days=days)).isoformat()
        result = (
            await db.table("llm_usage_stats")
            .select("model_code,total_input_tokens,total_output_tokens,total_calls,total_cost,success_count,fail_count,avg_exec_time_ms")
            .eq("tenant_id", str(org_id))
            .gte("stat_date", since)
            .neq("model_code", "_all")
            .execute()
        ...
        for r in rows:
            ...
            if mc not in agg:
                agg[mc] = {
                    "model_code": mc, "requests": 0, "success_count": 0,
                    "error_count": 0, "total_tokens": 0, "input_tokens": 0,
                    "output_tokens": 0, "total_cost": 0.0, "avg_latency_ms": 0,
                    "_latency_sum": 0, "_latency_cnt": 0,
                }
            a = agg[mc]
