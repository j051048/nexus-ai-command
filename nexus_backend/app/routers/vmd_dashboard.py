...
async def get_dashboard_stats(
    ...
    try:
        ...
        compliance_count = (
            compliance_res.count if compliance_res.count is not None else 0
        )
...
async def get_vmd_roi(req: Request, user_id: str = Depends(get_current_user_id)):
    ...
    try:
        ...
        done_tasks = (
            await db.table("vmd_main_task")
            .select("id,scene_code,created_at")
            .eq("tenant_id", tenant_id)
            .eq("status", "done")
            .execute()
        )
        ...
        return api_success(
            data={
                ...
                "automation_rate": (
                    round((1 - AI_HOURS_PER_TASK / MANUAL_HOURS_PER_TASK) * 100, 1)
                    if done_count > 0
                    else 0
                ),
                ...
                "budget_utilization": (
                    round(total_cost_usd / budget_monthly * 100, 1)
                    if budget_monthly > 0
                    else 0
                ),
                "scene_savings": [
                    {
                        ...
                        "tasks": cnt,
                        "hours_saved": round(
                            cnt * (MANUAL_HOURS_PER_TASK - AI_HOURS_PER_TASK), 1
                        ),
                        "cost_saved_cny": round(
                            cnt
                            * (MANUAL_HOURS_PER_TASK - AI_HOURS_PER_TASK)
                            * HOURLY_RATE_CNY,
                            2,
                        ),
                    }
