...
export function useSaveAISettings() {
  ...
  return useMutation({
    mutationFn: async (settings: { base_url: string; api_key: string | null; model: string; behavior_preferences?: BehaviorPreferences }) => {
      ...
      if (!profileData || !(profileData as Record<string, unknown>).organization_id) throw new Error('用户组织信息缺失，请联系管理员');

      const organizationId = (profileData as Record<string, unknown>).organization_id as string;
