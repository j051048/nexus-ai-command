...
export function SolutionStageRail({ activeStage, completedStages, onStageChange }: SolutionStageRailProps) {
  return (
    <nav aria-label="方案作业阶段" className="overflow-x-auto border-y bg-background">
      <div className="grid min-w-[760px] grid-cols-6">
        {SOLUTION_STAGE_DEFINITIONS.map((stage, index) => {
          ...
          return (
            <button
              ...
              className={cn(
                'relative flex min-h-16 items-center gap-3 border-r px-4 text-left transition-colors last:border-r-0 hover:bg-muted/50',
                active && 'bg-primary/[0.045]',
              )}
              aria-current={active ? 'step' : undefined}
            >
              <span className={cn('flex h-6 w-6 shrink-0 items-center justify-center rounded-full border text-xs', completed && 'border-emerald-600 bg-emerald-600 text-white', active && !completed && 'border-primary bg-primary text-primary-foreground')}>
                {completed ? <Check className="h-3.5 w-3.5" /> : index + 1}
              </span>
              <span className="min-w-0">
                <span className={cn('block text-sm font-medium', active ? 'text-foreground' : 'text-muted-foreground')}>{stage.shortLabel}</span>
